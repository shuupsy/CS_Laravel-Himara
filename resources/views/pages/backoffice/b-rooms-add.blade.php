<form action="/admin/rooms" method='post' enctype="multipart/form-data">
    ...
    <div class='flex flex-col gap-3 my-6'>
        ...
        <div>
            ...
            <x-text-input id="name" class="block mt-1 w-72" type="text" name="name" :value="old('name')" required
                autofocus />
        </div>
        ...
        <div>
            ...
            <div class="flex flex-col text-center range-slider mt-2">
                <span id="rs-bullet" class="text-xl">1</span>
                <input id="rs-range-line" type="range" name="nb_persons" min="1" step="1" max="6">
            </div>
        ...
        <div class='room-tags'>
            ...
            @foreach ($tags as $tag)
                <label>
                    <input class='hidden' type="checkbox" name="tag" value="{{ $tag->id }}">
                    <span class='w-20 p-2 rounded-lg cursor-pointer '>{{ $tag->tag }}</span>
                </label>
            @endforeach
        </div>
        ...
        <div>
            ...
            @foreach ($options as $option)
                <div>
                    <input type="checkbox" name="tag" value="{{ $option->id }}">
                    <span class='w-20 p-2 rounded-lg cursor-pointer '>{{ $option->option_name }}</span>
                </div>
            @endforeach
